...
async def index(request: Request) -> HTMLResponse:
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js"></script>
    ...
    html += (
        "  <script>\n"
        "    (function() {\n"
        "      // Robust interactive background with GSAP fallback\n"
        "      const largeHeader = document.getElementById('large-header');\n"
        "      const canvas = document.getElementById('demo-canvas');\n"
        "      if (!largeHeader || !canvas) {\n"
        "        console.log('interactive header or canvas not found');\n"
        "        return;\n"
        "      }\n"
        ...
        "        points = [];\n"
        "        const stepX = Math.max(Math.floor(width / 20), 40);\n"
        "        const stepY = Math.max(Math.floor(height / 20), 40);\n"
        ...
        "          }\n"
        "        }\n"
        "        // find 5 closest for each point\n"
        "        for (let i = 0; i < points.length; i++) {\n"
        "          const p = points[i];\n"
        "          p.closest = points.slice().filter(q => q !== p).sort((a, b) => (a.x - p.x) ** 2 + (a.y - p.y) ** 2 - ((b.x - p.x) ** 2 + (b.y - p.y) ** 2)).slice(0, 5);\n"
        "          p.circle = { radius: 2 + Math.random() * 2, active: 0, draw: function() { if (!this.active) return; ctx.beginPath(); ctx.arc(p.x, p.y, this.radius, 0, Math.PI * 2); ctx.fillStyle = 'rgba(61,163,255,' + this.active + ')'; ctx.fill(); } };\n"
        "        }\n"
        "      }\n"
        "      function shiftPoint(p) {\n"
        "        if (window.gsap && typeof gsap.to === 'function') {\n"
        "          gsap.to(p, { duration: 1 + Math.random(), x: p.originX - 50 + Math.random() * 100, y: p.originY - 50 + Math.random() * 100, ease: 'circ.inOut', onComplete: () => shiftPoint(p) });\n"
        "        } else {\n"
        "          // fallback movement without GSAP\n"
        "          setTimeout(() => { p.x = p.originX - 50 + Math.random() * 100; p.y = p.originY - 50 + Math.random() * 100; shiftPoint(p); }, 800 + Math.random() * 800);\n"
        "        }\n"
        "      }\n"
        "      function drawLines(p) {\n"
        "        if (!p.active || !p.closest) return;\n"
        "        p.closest.forEach(cp => { ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(cp.x, cp.y); ctx.strokeStyle = 'rgba(61,163,255,' + p.active + ')'; ctx.lineWidth = 1; ctx.stroke(); });\n"
        "      }\n"
        "      function animate() {\n"
        "        if (animateHeader) { ctx.clearRect(0, 0, width, height); points.forEach(p => { const d = (p.x - target.x) ** 2 + (p.y - target.y) ** 2; if (d < 4000) { p.active = 0.3; p.circle.active = 0.6; } else if (d < 20000) { p.active = 0.1; p.circle.active = 0.3; } else if (d < 40000) { p.active = 0.02; p.circle.active = 0.12; } else { p.active = 0; p.circle.active = 0; } drawLines(p); p.circle.draw(); }); } requestAnimationFrame(animate);\n"
        "      }\n"
        "      function initialise() {\n"
        "        resizeCanvas();\n"
        ...
        "      }\n"
        "      window.addEventListener('mousemove', function(e) { const rect = largeHeader.getBoundingClientRect(); target.x = e.clientX - rect.left; target.y = e.clientY - rect.top; });\n"
        "      window.addEventListener('resize', function() { initialise(); });\n"
        "      window.addEventListener('scroll', function() { animateHeader = window.scrollY <= height; });\n"
        "      initialise();\n"
